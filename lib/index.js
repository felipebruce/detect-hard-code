      ...
      create(context) {
        ...
        function callExpressionArguments(node) {
          node.arguments.forEach(arg => {
            if((arg.type === 'Literal' || arg.type === 'TemplateLiteral') &&  node.callee.name 
        ...
        /**
         * Ignore Angular translate service function instant parameter ias h
         * @param {ASTNode} node The node to check.
         * @returns {void}
         * @private
         */
        function angularInstantCase(node) {
          if(!node.callee.name) {
            node.arguments.forEach(arg => {
              if((arg.type === 'Literal' || arg.type === 'TemplateLiteral' && node.callee.property.name !== 'instant') && !hasRegex(arg) 
               &&  isCalleeTarget(node)
        ...
        return {
          ...
          Property: function (node) {
            ...
            if ((node.value.type === 'Literal' || node.value.type === 'TemplateLiteral') && !isBoolean(node.value) && typeof node.value.value !== 'number' && node.value.value !== '') {
              context.report({
                node,
                loc: node.value.loc,
                message: "Hardcoded string detected"
